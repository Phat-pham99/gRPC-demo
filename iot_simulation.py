...
# 4KB string block mimicking heavy binary metric tables / raw telemetry dumps
RAW_BYTES: bytes = os.urandom(1024 * 3)
BIG_PAYLOAD_STRING: str = base64.b64encode(RAW_BYTES).decode("utf-8")
DEVICE_NUMBER: int = int(os.environ.get("DEVICE_NUMBER", "20"))
PACKAGE_NUMBER: int = int(os.environ.get("PACKAGE_NUMBER", "1000"))
# Seconds to wait between frames per device
DELAY: float = float(os.environ.get("DELAY", "0.005"))
WS_URL: str = os.environ.get("WS_URL", "ws://127.0.0.1:3000")
...
async def main():
    total_frames = DEVICE_NUMBER * (PACKAGE_NUMBER + 1)
    print(f"Initializing {DEVICE_NUMBER} Simulated IoT System Devices...")
    print(f"Will send: {total_frames:,} packets total.")
    print(f"Delay {DELAY}s between packages")
